...
will attempt to load the package named in C<desired_version_from>, and
use it's version number (specified by it's C<$VERSION> global variable)
as the version number we wish to migrate to. This can be a handy way to keep
your schema versions in sync with your package versions.
...
My wife, for putting up with me while I coded this!
